...
def plot_iterations_train_error(data, filename=None, title=None):
    ...
    for i, opt in enumerate(all_optimizers):
        if opt in data and 'iterations' in data[opt]:
            ...
            # 计算均匀分布的标记点
            n = len(iterations)
            markevery_val = np.linspace(0, n-1, 13, dtype=int) if n >= 13 else slice(None)
    ...
    ax.yaxis.set_minor_locator(mpl.ticker.LogLocator(base=10.0, subs=[2, 5, 8]))
    ...
    if data:
        ...
        ax.set_ylim(min_train-0.6, min(1e5, max_train*1.2))
...
if __name__ == "__main__":
    try:
        ...
        data = load_data_from_csvs(
            ...
            pattern='TIMIT_Errors_*.csv'
            # pattern='sp500_Errors_*.csv'
        )
