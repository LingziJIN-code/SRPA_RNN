...
# 创建文件并写入表头 
filename = 'parameter_tuning_detailed_SPLQP_averaged10.csv'
fieldnames = [
...
with open(filename, 'a', newline='') as csvfile:  # 以追加模式打开文件
                ...
                for eta1 in param_space['eta1']:
                    for eta2 in param_space['eta2']: 
                        ...
                        for repeat in range(num_repeats):
                            ...
                            # 设置随机种子（每次使用不同的种子初始化&子问题抽片）
                            np.random.seed(123456 + repeat * 10)
                            random.seed(123456 + repeat * 10) 

                            results = SPLQP_ReLU_optimization(dataset_name=dataset_name, Ny=Ny, Nh=Nh, maxiter=50, 
                            distribution_type=init_method, mean=0, std=stddev, 
                            beta1=1, beta2=1, lambda1 = 1.2 / (Nh * Ny), lambda2 = 1.2 / (Nh * Nh), lambda3 = 1.2 / (Nx * Nh), lambda4 = 1.2 / Nh, lambda5 = 1.2 / Ny, 
    ...
    best_params_df.to_csv('best_SPLQPaveraged10_parameters.csv', index=False) 
