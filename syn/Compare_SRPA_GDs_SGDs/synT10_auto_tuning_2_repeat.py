...
def load_and_prepare_data():
    """加载并准备数据"""
    # 加载合成数据
    SynData = np.loadtxt(f"SynDataset_Nh{Nh}_Nx{Nx}_Ny{Ny}_T{T_total}_new.txt", delimiter=",")
    x_dataset = SynData[:, 0:Nx]
    y_dataset = SynData[:, Nx:]
    
    # 分割训练测试集 
    train_length = int(0.8 * T_total)
    ...
    x_input = convert_and_reshape(x_trainset, [T, 1, Nx])
    x_test_input = convert_and_reshape(x_testset, [T_test, 1, Nx])
    y_input = convert_and_reshape(y_trainset, [T, 1, Ny])
    y_test_input = convert_and_reshape(y_testset, [T_test, 1, Ny])
    
    # 调整形状
    x_input = tf.transpose(x_input, [1, 0, 2])
    x_test_input = tf.transpose(x_test_input, [1, 0, 2])
    y_input = tf.transpose(y_input, [1, 0, 2])
    y_test_input = tf.transpose(y_test_input, [1, 0, 2])
    
    return (x_trainset, y_trainset, x_testset, y_testset, T, T_test, 
...
def define_parameter_space():
    """定义参数搜索空间"""
    param_space = {
        'init_methods': ['Gaussian', 'He', 'Glorot', 'LeCun'],
        'gaussian_stddevs': [0.001, 0.01, 0.1],
        'learning_rates': [1e-4, 1e-3, 1e-2, 1e-1, 1.0],
        # 'clipnorm_values': [0.5, 1, 1.5, 2, 3, 4, 5, 6],
...
def create_and_train_model(optimizer_name, init_method, stddev, lr, clipnorm, batch_size, 
                          x_input, y_input, x_test_input, y_test_input, T, T_test,
                          eval_mode=False):
    """创建并训练模型 - 10次循环取平均"""
    # 设置随机种子
    ...
    for iteration in range(num_iterations):
        ...
        # 训练参数
        epochs = 50 if eval_mode else 500  # 评估模式使用较少epochs
        ...
        if optimizer_name in ['GD', 'GDC', 'GDNes']:
            ...
            train_gen = CustomDataGenerator(x_trainset, y_trainset, batchsize=batch_size, timelength=T)
...
#%% 参数调优主函数
def tune_optimizer_parameters(optimizer_name, x_input, y_input, x_test_input, y_test_input, 
                             T, T_test):
    """为指定优化器调优参数 - 使用遍历搜索"""
    print(f"\n{'='*50}")
        ...
        for stddev in stddev_choices:
            ...
            for lr in param_space['learning_rates']:
                ...
                if optimizer_name in ['GD', 'GDNes']:
                    # GD和GDNes: batch_size固定为T, clipnorm固定为None
                    batch_size_choices = [T]
                    clipnorm_choices = [None]
                elif optimizer_name == 'GDC':
                    # GDC: batch_size固定为T, clipnorm遍历所有值
                    batch_size_choices = [T]
...
def main():
    ...
    # 加载数据
    global x_trainset, y_trainset, x_testset, y_testset, T, T_test
    (x_trainset, y_trainset, x_testset, y_testset, T, T_test,
     x_input, y_input, x_test_input, y_test_input) = load_and_prepare_data()
    
    print(f"数据加载完成:")
    print(f"  训练集大小: {T}")
    print(f"  测试集大小: {T_test}")
    ...
    for optimizer in optimizers:
        best_config, trial_results = tune_optimizer_parameters(
            optimizer, x_input, y_input, x_test_input, y_test_input, T, T_test
        )
    ...
    trial_df.to_csv('parameter_tuning_detailed_results.csv', index=False)
    ...
    if best_params:
        ...
        best_params_df.to_csv('best_optimization_parameters.csv', index=False)
